<?php
    $datass=DB::select('select * from notebook where laptop_id = ?',[e($name)]);
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
	<div class="container">
	@include('errors')
	<form class = "form-signup" action = "{{url('/upd_laptop')}}" method = "post">
	 {{ csrf_field() }}
		<h2 class="form-signin-heading">Sign up</h2>
		<table>
			<div class="form-group">
			    <input type="hidden" class="form-control" name="ids"  value={{$name}} required autofocus>
			  </div>
			<div class="form-group">
			    <label>Model</label>
